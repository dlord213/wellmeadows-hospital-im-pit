  ...
  <div class="flex flex-row md:justify-between justify-center md:items-end items-center lg:max-w-7xl md:h-fit h-[100vh] p-4 mx-auto w-full bg-slate-200 rounded-br-lg rounded-bl-lg">
    <div class="flex flex-col">
      ...
      <?php if ($staff_position == 'Doctor') : ?>
        <p class="text-slate-600"><b><?= htmlspecialchars($doctor_fullname) ?></b> (ID: D<?= htmlspecialchars($doctor_id) ?>)</p>
      ...
      <?php elseif ($staff_position == 'Charge Nurse') : ?>
        <p class="text-slate-600"><b><?= htmlspecialchars($staff_name) ?></b> (S - <?= htmlspecialchars($staff_number) ?>) / <?= htmlspecialchars($staff_position) ?></p>
      ...
      <?php elseif ($staff_position == 'Personnel Officer') : ?>
        <p class="text-slate-600"><b><?= htmlspecialchars($staff_name) ?></b> (S - <?= htmlspecialchars($staff_number) ?>)<br> <?= htmlspecialchars($staff_position) ?></p>
      <?php else : ?>
        <p class="text-slate-600"><b>Staff:</b> <?= htmlspecialchars($staff_name) ?> (S - <?= htmlspecialchars($staff_number) ?>) / <?= htmlspecialchars($staff_position) ?></p>
    ...
    <div class="hidden md:flex flex-row items-center gap-4">
      <?php if ($staff_position == "Medical Director" or $staff_position == "Personnel Officer") : ?>
        <form action="<?= htmlspecialchars($_SERVER["PHP_SELF"]) ?>" method="GET" name="ward_form" class="lg:block hidden">
        ...
        <div class="lg:block hidden bg-slate-500 w-[2px] h-[32px] rounded-lg"></div>
      <?php endif; ?>
